...
import { albumRepo, trackRepo, playbackRepo } from '@khinsider/core';
...
export class NavigationPanel {
      ...
      if (isExpanded && this.albums[year]) {
        for (const album of this.albums[year]) {
          ...
          items.push({
            type: 'album',
            album: album,
            text: `${albumPrefix} ${playedIcon}${favIcon}${escapeBlessedMarkup(album.title)}`
          });
          ...
          if (isAlbumExpanded && this.tracks[album.id]) {
            for (let i = 0; i < this.tracks[album.id].length; i++) {
              ...
              items.push({
                ...
                track: track,
                trackIndex: i,
                text: `    ${trackPlayedIcon}${num}. ${escapeBlessedMarkup(track.name)}`
              });
            }
          }
        }
  ...
  updateLabel(): void {
    ...
    if (this.searchMatches.length > 0) {
      ...
      label = ` ${this.selectedYear} > ${this.truncate(escapeBlessedMarkup(this.selectedAlbum.title), 25)} `;
