...
import { formatDuration, truncate, escapeBlessedMarkup } from '../utils/formatters.js';
import { PlaybackState } from '@khinsider/shared';
import type { Track, Album, AlbumInfo, PlaybackStateType } from '@khinsider/shared';
...
export class NowPlayingPanel {
  ...
  getTrackContent() {
    ...
    const trackName = escapeBlessedMarkup(this.currentTrack?.name) || 'Unknown Track';
  ...
  getAlbumContent() {
    ...
    const albumTitle = escapeBlessedMarkup(this.currentAlbum?.title) || 'Unknown Album';
    const lines = [
      `${favIcon} ${downloadIcon}  {bold}{cyan-fg}${truncate(albumTitle, 40)}{/cyan-fg}{/bold}`
    ];

    // Show metadata lines in original format
    if (this.albumMeta?.metadataLines && this.albumMeta.metadataLines.length > 0) {
      lines.push('');
      for (const item of this.albumMeta.metadataLines) {
        const label = escapeBlessedMarkup(item.label);
        const value = escapeBlessedMarkup(item.value);
        lines.push(`{gray-fg}${label}:{/gray-fg} {white-fg}${value}{/white-fg}`);
      }
    } else {
    ...
    if (this.albumMeta?.images && this.albumMeta.images.length > 0) {
      lines.push('');
      ...
      if (this.imagesExpanded && this.albumMeta.images) {
        // Show each image on its own line when expanded
        this.albumMeta.images.forEach((url, i) => {
          // SECURITY: Wrap decodeURIComponent in try-catch to handle malformed URLs
          let name: string;
          try {
            name = decodeURIComponent(url.split('/').pop() || '');
          } catch {
            // Fall back to raw filename if decoding fails
