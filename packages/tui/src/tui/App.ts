...
import { settingsRepo } from '@khinsider/core';
import {
  getErrorMessage,
  ...
  type AlbumInfo,
  type YearInfo,
  type TrackStartEventData,
  type TrackCompletedEventData,
  type AlbumEventData,
  type AlbumLoadedEventData,
  type YearEventData,
  type YearLoadedEventData,
  type ErrorEventData,
  type DownloadEventData,
  type DownloadProgressEventData,
  type DownloadErrorEventData
...
import type { TrackRepository } from '@khinsider/core';
import type { AlbumDownloader } from '@khinsider/core';
...
export class App {
  ...
  _globalKeyHandler: ((ch: string | null, key: blessed.Widgets.KeyEventData) => void) | null;
  ...
  /**
   * Cleanup resources after partial initialization failure
   */
  private cleanupPartialInit(): void {
    // Remove global key handler if registered
    if (this._globalKeyHandler && this.screen) {
      try {
        this.screen.removeListener('keypress', this._globalKeyHandler);
      } catch { /* ignore */ }
      this._globalKeyHandler = null;
    }

    // Cleanup playback event listeners
    if (this._pcListeners && this.playbackController) {
      try {
        for (const [event, listener] of Object.entries(this._pcListeners)) {
          this.playbackController.off(event, listener);
        }
      } catch { /* ignore */ }
      this._pcListeners = null;
    }

    // Cleanup downloader listeners
    if (this._dlListeners && this.downloader) {
      try {
        for (const [event, listener] of Object.entries(this._dlListeners)) {
          this.downloader.off(event, listener);
        }
      } catch { /* ignore */ }
      this._dlListeners = null;
    }
  ...
  async tryAutoLogin() {
    ...
    if (credentials) {
      ...
      try {
        ...
        // SECURITY: Don't display username in logs to prevent information disclosure
        this.panels.statusBar.showSuccess('Successfully logged in');
        this.panels.history.logInfo('Login successful');
  ...
  setupPlaybackEvents() {
    ...
    this._pcListeners = {
      ...
      trackCompleted: (data: unknown) => {
        ...
        this.panels.navigation.markTrackCompleted(d.track.id, d.album?.id ?? 0);
  ...
  async toggleFavorite() {
    ...
    const message = `${action} favorites?\n\n{cyan-fg}${escapeBlessedMarkup(album.title)}{/cyan-fg}`;
  ...
  showConfirm(message: string, callback: (confirmed: boolean) => void): void {
    ...
    const handleKey = (ch: string, key: blessed.Widgets.KeyEventData) => {
      if (handled) return;
      if (key.name === 'y') {
        cleanup();
  ...
  showLoginDialog() {
    ...
    if (isLoggedIn) {
      ...
      // SECURITY: Display login status without exposing full username
      const maskedUser = credentials?.username
        ? credentials.username.charAt(0) + '***'
        : 'Unknown';
      dialogBox.setContent(`
  {bold}Khinsider Login{/bold}

  Status: {green-fg}Logged in (${escapeBlessedMarkup(maskedUser)}){/green-fg}
      ...
      showLoginForm({
        ...
        onSubmit: async (username, password) => {
          ...
          try {
            ...
            // SECURITY: Don't include detailed error messages that could leak info
            this.panels.statusBar.showError('Login failed');
            this.panels.history.logError('Login failed');
          }
        },
        onCancel: () => {
